...
memetic g = [("AM-(10,0.1mej)", memeticMej g),
...
getBest g s = getV $ S.findMax pop
 where (_, pop) = evalState s (g,0)

iterAlgo step g ds = getBest g $ untilM (\_ -> (> 15000) <$> gets snd) (step ds) (startMem ds)
