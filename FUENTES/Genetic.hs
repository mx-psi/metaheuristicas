...
genetic g =
  [ ("Stationary BLX"         , stationBLX g)
  , ("Stationary Arithmetic"  , stationAr g)
  , ("Generational Arithmetic", generAr g)
  , ("Generational BLX"       , generBLX g)
  ]
...
duel pop = maximum <$> replicateM size (sample pop) where size = 2
...
getBest g s = getV $ S.findMax pop where pop = evalState s (g, 0)

iterAlgo step g ds =
  getBest g $ untilM (\_ -> (> 15000) <$> gets snd) (step ds) (randPop 30 ds)
...
contend ds cs pop = do
  ...
  pure $ S.deleteMin $ S.deleteMin (children `S.union` pop)
...
mutGener pop = do
  ...
  pure $ S.insert new $ S.deleteAt j pop
...
buildPop ds mutPop = do
  ...
  pure $ old `S.union` evaluated
  where (new, old) = S.spanAntitone ((==) (-1) . getEval) mutPop

generBLXStep ds pop = do
  children <- concat
    <$> replicateM nCross (replicateM 2 (duel pop) >>= crossBLX)
  ...
  pure (S.deleteMin $ S.insert best finalPop)
 where
  nMut   = S.size pop * nFeats ds `quot` 1000
  nCross = floor (fromIntegral (S.size pop) * 0.7) `quot` 2
  best   = S.findMax pop

crossAr :: [Solution] -> [Solution]
crossAr ss = map (Sol (-1) 0)
                 [arith p1 p2, arith p2 p3, arith p3 p4, arith p4 p1]
...
generArStepMemetic ds pop = do
  ...
  pure (S.deleteMin $ S.insert best finalPop)
 where
  nMut   = S.size pop * nFeats ds `quot` 100
  nCross = 2
  best   = S.findMax pop
