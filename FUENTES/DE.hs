...
deSol ds v = incEval >> pure (DESol (rawEval v ds) v)
...
pickDistinctFrom pop i n = map (\k -> pop V.! k) <$> indices
 where
  indices = (take n . filter (/= i) . nub) <$> randRs (0, V.length pop - 1)
...
getBest g s = deV $ maximum pop where pop = evalState s (g, 0)
...
deRand g ds =
  getBest g $ untilM stopCondition (deRandStep ds) (randDEPop 50 ds)
deBest g ds =
  getBest g $ untilM stopCondition (deBestStep ds) (randDEPop 50 ds)
...
updateRand
  :: DataSet -> DEPopulation -> Int -> Int -> DESolution -> Rand DESolution
...
crossDeBest neighbors jrand best i xi = do
  ...
  if ri < cr || i == jrand
    then pure $ min 1 $ max 0 $ xi + f * (xbest - xi) + f * (a - b)
    else pure xi
 where
  cr = 0.5 :: Double
  f  = 0.5 :: Double
