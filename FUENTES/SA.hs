...
cool :: SAData -> SAData
cool saData@(SA { getK = k, getT = cT, beta = b }) = saData { getK = k + 1
                                                            , getT = coolT b cT
                                                            }
...
resetC saData = saData { nSuccess = 0, nNeighbors = 0 }
...
sa :: StdGen -> Algorithm
sa g ds = getWs g $ getBest <$> untilM stopCondition
                                       (saStep ds)
                                       (initial <$> randSol ds)
...
stopCondition saData = do
  nEvals <- gets snd
  pure (nEvals >= 15000 || nSuccess saData == 0)
...
exploreNeighbors ds = untilM checkStop (genNeighbor ds) . pure
 where
  checkStop sd = pure
    ( nNeighbors sd > maxNeighbors (getSol sd) || nSuccess sd > maxSuccess
      (getSol sd)
    )
...
genNeighbor ds sd = do
  ...
  if dif < 0 || r <= exp (-dif / (k * t))
    then pure (setCur s' sd)
    else pure (inc sd)
 where
  s = getSol sd
  v = getV s
